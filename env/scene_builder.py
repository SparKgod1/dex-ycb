...
# my custom table scene to fit my data of dexycb
# according to
# https://github.com/dexsuite/dex-retargeting/blob/6025f92e8686a608598239f1d6600d6761b43646/example/position_retargeting/hand_viewer.py#L88-L108
class TableSceneBuilderDEXYCB(SceneBuilder):
    def build(self):
        builder = self.scene.create_actor_builder()
        # model_dir = Path(osp.dirname(__file__)) / "assets"
        # table_model_file = str(model_dir / "table.glb")
        ...
        # )
        builder.add_box_collision(
            pose=sapien.Pose([0.0964334011077881, 0.3492412269115448, -0.02]),
            half_size=np.array([0.3, 0.3, 0.02]),
        )
        white_diffuse = sapien.render.RenderMaterial(emission=[0.0, 0.0, 0.0, 0.0])
        ...
        if self.scene.parallel_in_single_scene:
            floor_width = 500
        self.ground = build_ground(
            self.scene, floor_width=floor_width, altitude=-1
        )
        self.table = table
        self.scene_objects: List[sapien.Entity] = [self.table, self.ground]
